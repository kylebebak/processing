...
float thresholdValue = .235;

float currentSize;
float initialSize = 335;
float minSize = 15.5;
float shrinkingFactor = .9875;
int characterSpacing = 1;
int fontSize = round(.75 * initialSize);
int numberAttemptsToDrawLetter = 300;
...
float letterHue = PI / 7.5;
float hueRange = PI / 7.5;


void setup() {
  //  Road_Works.svg.png, face1.jpg
  backgroundImage = loadImage("/Users/kylebebak/Desktop/Dropbox/Programming/Processing/Rascius sketches/Space Filling/space_or_shape_filling_based_on_rascius_color/kyle_miriam2.jpeg");
...
void draw() {
  ...
  if (currentSize > minSize) {
    ...
    PGraphics pdf = createGraphics(width, height, PDF, "output.pdf");
    pdf.beginDraw();
    pdf.image(get(), 0, 0);
    pdf.dispose();
    pdf.endDraw();
