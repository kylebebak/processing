...
void setup() {
  ...
  + "Rascius sketches/Space Filling/space_filling_and_disintegration_P3D/kyle_face1.jpeg");
...
void initialize() {
  ...
  backgroundLayer.background(color(0, 0, 1));
    ...
    for (int dx = 0; dx<intLetterSize && fits; dx++) {
      for (int dy = 0; dy<intLetterSize && fits; dy++) {
        if (brightness(gMask.get(dx, dy))<0.5) {
          if (brightness(get(pos[0]+dx, pos[1]+dy))<0.5 || brightness(backgroundImage.get(pos[0]+dx, pos[1]+dy))<0.5) {
            fits = false;
...
class Letter {
  ...
  void eraseStaticAndInitializeDynamic() {
    ...
    // like it was a sprite, with the letter totally opaque on top of the totally
    //  transparent background
    staticLetterImage.fill(color(0, 0, 1));
  ...
  boolean updateDynamic() {
    ...
    dynamicLetterImage.fill(color(0, 0, 0, max(.01, 1-transparency)));
