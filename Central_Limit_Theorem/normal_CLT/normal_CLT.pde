...
void draw() {
  ...
  float maxsum=range*variables;
  float ratio=resf/(maxsum/zoom);
  ...
  float bell0=pow(1+exp(-(1/s)*(0-u)),-1);
    ...
    for (int j=0; j<variables; j=j+1) {
      ...
      //interesting random variable, symmetric about u with width parameter s,
      //definitely not guassian
      //r=random(0,1);
    ...
    int xsumi=floor(ratio*xsum);
    if ((xsumi<=resolution-1) && (xsumi>-1)) {
      frequency[xsumi]=frequency[xsumi]+1;
    }
  ...
  String upper = "upper = " + nf(maxsum/zoom, 1, 5);
  ...
  text(nf((maxsum/zoom)*posf/resolutionf, 1, 2), posf*width/resolutionf, .25*height-10);
}
