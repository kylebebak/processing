...
public class Tour {
  ...
  public void showTour() {
    ...
    println(first.p.toString());
    for (Node n = first.next; n != first; n = n.next)
      println(n.p.toString());
    println(first.p.toString());
  }
  ...
  public void drawTour(float maxDist) {
    if (N < 2)
      return;

    first.p.drawTo(first.next.p);
    for (Node n = first.next; n != first; n = n.next) if (n.p.distanceTo(n.next.p) < maxDist) n.p.drawTo(n.next.p);
  }
