...
void setup() {
  ...
  pg=createGraphics(width/3, height/3, JAVA2D);
...
void draw() {
  ...
  /*when background is called, it's not equivalent to calling a
   a rectangle with the width and height of the screen. rather,
   it gets rid of the color information for all the pixels on the screen
   and replaces it with the color specified in the background call.
...
drawBranch(1.3*pg.width, 1.2*pg.height, .4*pg.width, -.4*pg.height, 1.2, 
  .01, 1, 1.5*sq( -1 + noise(0, 0, frameCount*.025) + noise(0, frameCount*.025, 0) ), 
  2, 7, 5, 16, pg );
pg.filter(BLUR, 1);
