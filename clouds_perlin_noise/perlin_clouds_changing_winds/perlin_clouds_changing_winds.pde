...
PImage img;
float n, nbx, nby;
//noise for drawing clouds and also changing breeze direction
float b=.03, bx=0, by=0;
//breeze strength for a "breeze" that moves the clouds
float mx, my;
float md=80;
...
void draw() {
  ...
  nbx=noise(.001*frameCount, .002*frameCount);
  nby=noise(.002*frameCount, .001*frameCount);
  bx=b*(nbx-.5);
  by=b*(nby-.5);
  ...
  for (int j=0; j<img.height; j++) {
    for (int i=0; i<img.width; i++) {
      ...
      /*positive values for bx and by will make pixels
       ...
       also, using 3 dimensional noise with the third dimension
       depending only on the frameCount will make the shape
       of the moving clouds change with time*/

      img.pixels[i+j*width] = color(255, 255*n*n*n);
