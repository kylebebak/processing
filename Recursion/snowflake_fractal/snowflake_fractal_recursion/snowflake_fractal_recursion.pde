...
float[] triangleSizes = {600.0, 300.0, 150.0};
...
void setup() {
  ...
  drawTriangleFractal(generateTriangleVertices(triangleSizes[0], triangleCenter), 4);
  drawTriangleFractal(generateTriangleVertices(triangleSizes[1], triangleCenter), 4);
  drawTriangleFractal(generateTriangleVertices(triangleSizes[2], triangleCenter), 4);
}

void draw() {
  
  for (int i=0; i<triangleSizes.length; i++) {
   triangleSizes[i] += triangleSizes[i]*random(-.02, .02);
  }
  noStroke();
  fill(0, 30);
...
float[] generateTriangleVertices(float sideLength, float[] triangleCenter) {
  float[] v = new float[6];
...
void drawTriangleFractal(float[] vertices, int iterations) {
  float[] v = vertices;
