...
void pushOutTriangle(float x0, float y0, float x1, float y1, int iterations, boolean initialize) {
  ...
  if (initialize) {
    float[] info = {
      random(60), random(.15, .45), random(.55, .85), 
  ...
  if (!initialize) {
    ...
    //    currentInfo[0] = mod(currentInfo[0], 360);

    currentInfo[0] += random(-5, 5);
    currentInfo[0] = mod(currentInfo[0], 60);
  ...
  if (counter == 1) println(currentInfo[0]);



  float len = dist(x0, y0, x1, y1);
  float colorTemp = currentInfo[0];
  if (colorTemp < 30) colorTemp += 330;
